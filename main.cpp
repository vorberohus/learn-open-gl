...
static void
copyInputState(input_state *source, input_state *dest)
{
...
static input_delta
getInputStateDelta(input_state *current, input_state *prev)
{
    input_delta result = {};
...
static bool32
CreateVertexProgram(const char *vertFileName, _Out_ uint32 *o_vertShader)
{
...
static bool32
CreateFragmentProgram(const char *fragFileName, _Out_ uint32 *o_fragShader)
{
...
static bool32
CreateShaderProgram(uint32 vertShader, uint32 fragShader, _Out_ uint32 *o_shaderProgram)
{
    *o_shaderProgram = 0;
...
static bool32
CreateShaderProgram(const char *vertFileName, const char *fragFileName, _Out_ uint32 *o_shaderProgram)
{
    uint32 vertShader;
    uint32 fragShader;
...
class Camera
{
  ...
  public:
    Camera(glm::vec3 position, glm::vec3 direction, glm::vec3 up, float fov, float aspectRatio, float nearPlane,
           float farPlane, float speed)

        : position(position), direction(direction), up(up), fov(fov), aspectRatio(aspectRatio), nearPlane(nearPlane),
          farPlane(farPlane), yaw(glm::degrees(atan2(direction.z, direction.x))),
    ...
    void
    SetDirection(const glm::vec3 &newDirection)
    {
    ...
    void
    SetYawAndPitch(float newYaw, float newPitch)
    {
        yaw = newYaw;
        pitch = newPitch;
    ...
    void
    AddYawAndPitch(float addYaw, float addPitch)
    {
        yaw += addYaw;
        pitch += addPitch;
...
static void
updateCameraPosition(Camera *camera, const input_state *inputState, float deltaTime)
{
...
static void
printDebugInfo(float deltaTime)
{
    input_state *inputState = gameState.currentInput;
    ...
    std::cout << "Mouse pos normalized: " << inputState->normalizedMousePosX << ' ' << inputState->normalizedMousePosY
              << '\n';
    std::cout << "\r\033[K";
    std::cout << "Mouse delta: " << gameState.inputDelta.mouseDeltaX << ' ' << gameState.inputDelta.mouseDeltaY << '\n';
    std::cout << std::flush;
}
...
main()
{
    ...
    Camera cam = Camera(glm::vec3(0.0f, 0.0f, 5.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f), 45.0f,
                        1920.0f / 1080.0f, 0.1f, 100.0f, 2.5f);
